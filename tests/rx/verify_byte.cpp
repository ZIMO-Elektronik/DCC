...
TEST_F(RxTest, verify_byte_operations_mode) {
  ...
  Expectation read_cv{EXPECT_CALL(mock_, readCv(cv_addr, 0u))};
...
TEST_F(RxTest, verify_byte_service_mode) {
  ...
  auto cv_addr{RandomInterval(0u, smath::pow(2u, 10u) - 1u)};
  Expectation read_cv{
    EXPECT_CALL(mock_, readCv(cv_addr, 42u)).WillOnce(Return(42u))};
  Expectation ack{EXPECT_CALL(mock_, serviceAck())};
