...
void add_loco_packets(State& state) {
  ...
  state.op_packets.push_back({.bytes = dcc::make_set_advanced_addressing_packet(
  ...
  state.op_packets.push_back({.bytes = dcc::make_time_scale_packet(
  ...
  // CV access
  state.op_packets.push_back({.bytes = dcc::make_cv_access_long_verify_packet(
                                random_loco_address(),
                                random_interval<uint16_t>(0u, 1023u),
                                random_interval<uint8_t>())});
  state.op_packets.push_back({.bytes = dcc::make_cv_access_long_write_packet(
                                random_loco_address(),
                                random_interval<uint16_t>(0u, 1023u),
                                random_interval<uint8_t>())});
  state.op_packets.push_back({.bytes = dcc::make_cv_access_long_verify_packet(
...
void add_accessory_packets(State& state) {
  ...
  state.op_packets.push_back({.bytes = dcc::make_accessory_nop_packet(
